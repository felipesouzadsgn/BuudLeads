...
const Button = ({ 
  ...
  const baseStyles = "inline-flex items-center justify-center rounded-lg px-6 py-3 text-sm font-bold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary focus:ring-offset-background-dark";
  
  const variants = {
    primary: "bg-primary text-background-dark hover:opacity-90 shadow-[0_0_20px_rgba(0,191,255,0.3)] hover:shadow-[0_0_30px_rgba(0,191,255,0.5)] hover:-translate-y-0.5",
    outline: "bg-transparent border border-surface-dark text-white hover:bg-surface-dark hover:border-white/40 hover:-translate-y-0.5",
...
const SectionHeading = ({ title, subtitle, badge }: { title: string, subtitle?: string, badge?: string }) => (
  <div className="flex flex-col items-center text-center gap-4 mb-16 animate-slide-up">
    ...
    <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white max-w-4xl leading-tight">
      {title}
    </h2>
    {subtitle && (
...
const Logo = () => (
  <div className="flex items-center gap-2 font-bold text-xl tracking-tight text-white select-none">
    <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center transform rotate-3 shadow-[0_0_15px_rgba(0,191,255,0.5)]">
      <div className="w-4 h-4 bg-background-dark rounded-sm transform -rotate-3" />
    </div>
    BUUDLEADS
  </div>
);
...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background-dark/80 backdrop-blur-xl border-b border-white/5 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Logo />
        
        {/* Desktop Nav */}
        <nav className="hidden md:flex items-center gap-8">
          <a href="#benefits" className="text-sm font-medium text-text-muted hover:text-white transition-colors">Benefícios</a>
          <a href="#plans" className="text-sm font-medium text-text-muted hover:text-white transition-colors">Planos</a>
          <a href="#faq" className="text-sm font-medium text-text-muted hover:text-white transition-colors">FAQ</a>
        </nav>
      ...
      {/* Mobile Nav */}
      {isOpen && (
        <div className="md:hidden absolute top-16 left-0 right-0 bg-background-dark/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-4 animate-fade-in shadow-2xl h-screen">
          <a href="#benefits" className="text-white text-lg font-medium py-3 border-b border-white/5" onClick={() => setIsOpen(false)}>Benefícios</a>
          <a href="#plans" className="text-white text-lg font-medium py-3 border-b border-white/5" onClick={() => setIsOpen(false)}>Planos</a>
          <a href="#faq" className="text-white text-lg font-medium py-3 border-b border-white/5" onClick={() => setIsOpen(false)}>FAQ</a>
  ...
  return (
    <section className="pt-32 pb-20 md:pt-48 md:pb-32 px-6 relative overflow-hidden flex flex-col items-center">
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center text-center z-10">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-dark/50 border border-white/10 text-sm font-medium text-primary mb-8 animate-fade-in backdrop-blur-md">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white tracking-tight leading-[1.1] mb-6 max-w-5xl animate-slide-up">
        ...
        <div className="mt-20 w-full max-w-6xl relative animate-slide-up delay-300 group perspective-1000">
          ...
          <div className="relative rounded-xl border border-white/10 bg-surface-dark overflow-hidden shadow-2xl transition-transform duration-700 transform group-hover:scale-[1.01]">
            <div className="aspect-[16/10] w-full bg-surface-dark relative flex items-center justify-center overflow-hidden">
               <img 
...
const Authority = () => {
  return (
    <section className="py-16 md:py-24 bg-surface-dark/30 border-y border-white/5 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 text-center relative z-0">
        <div className="flex flex-col items-center gap-4 mb-16">
          ...
          <h3 className="text-2xl md:text-4xl font-bold text-white">Aqui é jogo grande.</h3>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, idx) => (
            <div 
              key={idx} 
              className="group p-8 rounded-2xl bg-surface-dark border border-white/5 hover:border-primary/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-primary/5"
            >
              <div className="w-14 h-14 rounded-xl bg-background-dark border border-white/5 flex items-center justify-center mb-6 group-hover:border-primary/50 transition-colors group-hover:scale-110">
                {feature.icon}
              </div>
  ...
  return (
    <section className="py-16 md:py-24 px-6 relative">
      <div className="max-w-5xl mx-auto text-center">
        <div className="aspect-[16/9] w-full bg-surface-dark rounded-xl border border-white/10 p-2 shadow-2xl relative group overflow-hidden">
...
const Differentiators = () => {
  return (
    <section className="py-24 px-6 bg-surface-dark/20 border-y border-white/5">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
          {[
            ...
            <div key={i} className="flex flex-col items-center text-center p-8 bg-surface-dark border border-white/5 rounded-2xl hover:border-primary/20 transition-all hover:translate-y-1">
              {item.icon}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <div key={i} className="bg-surface-dark border border-white/10 p-8 rounded-2xl flex flex-col justify-between h-full hover:border-primary/30 transition-colors">
              <div>
                <div className="flex gap-1 mb-6">
                   {[1,2,3,4,5].map(star => <Star key={star} size={16} fill="currentColor" className="text-primary" />)}
                </div>
                <p className="text-lg text-white mb-8 italic leading-relaxed">"{t.text}"</p>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4 border-t border-white/10 flex items-center gap-2 text-green-400 text-sm font-semibold">
                  <TrendingUp size={16} />
                  {t.metric}
                </div>
              </div>
...
const Pricing = () => {
  ...
  return (
    <section id="plans" className="py-24 px-6 bg-surface-dark/30 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start max-w-6xl mx-auto">
          {plans.map((plan, i) => (
            <div 
              key={i} 
              className={`relative p-8 rounded-2xl flex flex-col transition-transform duration-300 ${
                plan.highlight 
                  ? "bg-surface-dark border-2 border-primary shadow-[0_0_40px_rgba(0,191,255,0.15)] scale-105 z-10" 
                  : "bg-surface-dark/50 border border-white/10 hover:border-white/20"
              }`}
            >
              {plan.highlight && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary text-background-dark text-xs font-bold uppercase py-1 px-4 rounded-full tracking-wider shadow-lg shadow-primary/30">
                  Mais Popular
                </div>
              )}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div 
              key={i} 
              className="bg-surface-dark border border-white/10 rounded-xl overflow-hidden transition-all duration-300 hover:border-white/20"
            >
              <button 
...
const Footer = () => {
  return (
    <footer className="bg-surface-dark border-t border-white/10 py-12 px-6">
...
function App() {
  return (
    <div className="min-h-screen bg-background-dark text-white selection:bg-primary selection:text-background-dark font-sans">
      <Header />
      <main>
        <Hero />
        <Authority />
